    ...
    Cli.Program.config
        |> Cli.Program.add
            (Cli.OptionsParser.build CliOptions
                ...
                |> Cli.OptionsParser.with
                    (Cli.Option.optionalKeywordArg "dump-merged-to")
...
options:
  ...
  --dump-merged-to                   Dump the merged Open API spec to the given file.
"""
            )
    ...
    Pages.Script.withCliOptions program
        (\cliOptions ->
            Pages.Script.Spinner.steps
                ...
                |> (case cliOptions.dumpMergedTo of
                        Nothing ->
                            identity

                        Just destination ->
                            Pages.Script.Spinner.withStep "Dumping merged OAS" (dumpMerged destination)
                   )
...
dumpMerged : String -> Json.Decode.Value -> BackendTask.BackendTask FatalError.FatalError Json.Decode.Value
dumpMerged destination spec =
